...
function killvm(){
		...
		dialog --title "Confirm" --yesno "Really power off $VMNAME?" 5 55
			...
			VBoxManage controlvm $VMNAME poweroff
        		dialog --title "Response" --msgbox "${VMNAME} is powering down." 5 55
		fi
        fi
}
function resetvm(){
		...
		dialog --title "Confirm" --yesno "Really reset $VMNAME?" 5 55
			...
			dialog --title "Response" --msgbox "${VMNAME} is restarting." 5 55
		fi
        fi
}
function startvm(){
        if [ $STATE = "running" ]
	 	...
	 	nohup VBoxHeadless --startvm $VMNAME & 1>/dev/nul 2>/dev/null
		dialog --title "Response" --msgbox "${VMNAME} is starting." 5 55
	fi
}
function sleepvm(){
	if [ $STATE != "running" ]
...
function mainmenu(){
	...
	dialog --clear  --help-button --backtitle "VirtualBox Command Menu" --title "[ VM Control Panel: Main Menu  ]" --menu "You can use the UP/DOWN arrow keys, the first \n letter of the choice as a hot key, or the \n number keys 1-9 to choose an option.\n\n\
